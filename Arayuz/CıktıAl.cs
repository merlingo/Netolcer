...
namespace Arayuz
{
    ...
    public void createPdf(FileStream fs,DateTime ilkTarih, DateTime sonTarih)
    {
        ...
        phraseHedeflenenOrtamKosullari.Add(new Chunk(options.HedeflenenOrtamKosullari, FONT[4]));
        ...
        string kayitSikligi = options.VeriAraligi < 60 ? options.VeriAraligi.ToString() + "dk" : (options.VeriAraligi / 60).ToString() + "saat";
        phraseVeriKayitSikligi.Add(new Chunk(kayitSikligi, FONT[4]));
        ...
        foreach (var dlicincikti in options.CiktiIcinDataLoggerlar)
        {
            ...
            foreach (DataRow row in _newDataRown)
            {
                ...
                {
                    if (rowDate == aralikBasi || (rowDate - aralikBasi).TotalMinutes >= options.VeriAraligi)
                    {
                        aralikBasi = rowDate;
    ...
    public static void tehlikeliDegerCikti(List<NetOlcerBirimi> degerler,string dataloggerIsim, string sensor, FileStream fs, string tehlikeYazisi)
    {
        ...
        string baslik = "NETÖLÇER - " + dataloggerIsim + " - " + sensor;
        Paragraph dokumanBasligi = new Paragraph(baslik, FONT[0]);
        Chapter chapterDokumanBaslikveGenelBilgiler = new Chapter(0);
        Section sectionBaslik = chapterDokumanBaslikveGenelBilgiler.AddSection(dokumanBasligi, 0);
        chapterDokumanBaslikveGenelBilgiler.NumberDepth = 0;
        if (degerler.Count <= 0)
        {
            Section sectionDataloggerBaslik = chapterDokumanBaslikveGenelBilgiler.AddSection(new Paragraph(tehlikeYazisi + " Tehlikeli Degerler", FONT[1]), 0);

            sectionDataloggerBaslik.NumberDepth = 0;
            sectionDataloggerBaslik.Add(new Paragraph("\n"));
            sectionDataloggerBaslik.Add(new Paragraph("Belirtilen set degeri için asim/dusum bulunamamistir."));
        ...
        {
            PdfPTable tableDataloggerVeri = new PdfPTable(4);
            string[] s = { "Sensor", "Deger", "Birim", "Zaman" };
            foreach (string column in s)
            {
                PdfPCell cell = new PdfPCell(new Phrase(column));
                tableDataloggerVeri.AddCell(cell);
            }
            foreach (NetOlcerBirimi deger in degerler)
            {

                PdfPCell cellSensor = new PdfPCell(new Phrase(deger.Sensor));
                tableDataloggerVeri.AddCell(cellSensor);
                PdfPCell cellDeger = new PdfPCell(new Phrase(deger.Deger.ToString()));
                tableDataloggerVeri.AddCell(cellDeger);
                PdfPCell cellBirim = new PdfPCell(new Phrase(deger.Birim));
                tableDataloggerVeri.AddCell(cellBirim);
                PdfPCell cellZaman = new PdfPCell(new Phrase(deger.Zaman.ToString("dd/MM/yyyy, HH:mm")));
                tableDataloggerVeri.AddCell(cellZaman);
    ...
    public class NetOlcerCiktiOptions 
    {
        ...
        public String HedeflenenOrtamKosullari { get; set; }
